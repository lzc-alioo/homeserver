...
if [ "" != "$pid" ]; then
    sudo kill -9 $pid
    echo "$(date "+%Y-%m-%d %H:%M:%S") === app:$RUN_APP event:stop === 执行cpolarstop.sh时计划kill掉进程pid:$pid,执行结果:$?"
else
    echo "$(date "+%Y-%m-%d %H:%M:%S") === app:$RUN_APP event:stop === 执行cpolarstop.sh时发现进程不存在，直接跳过"
fi
