...
//import com.alioo.monitor.machine.service.request.FlowStatisticDto;
...
//@Slf4j
////@Service
//public class TpLinkFlowStatisticService implements MachineService {
//
...
//                    continue;
//                }
//                FlowStatisticDto request = new FlowStatisticDto();
//                request.setId(Integer.parseInt(colarr[0]));
//                request.setIp(colarr[1]);
//                request.setMac(colarr[2]);
//                request.setDownloadTotal(Long.parseLong(colarr[3]));
//                request.setUploadTotal(Long.parseLong(colarr[4]));
//                request.setDownloadSpeed(Long.parseLong(colarr[5]));
//                request.setUploadSpeed(Long.parseLong(colarr[6]));
//
//                map.put(colarr[2], request);
//            }
//        } catch (Exception e) {
...
//
//
//    public boolean accessControl(FlowStatisticDto request) {
...
//
//        try {
//            Integer ctrlId = ctrlmap.get(request.getAlias());
//            Assert.notNull(ctrlId, request.getAlias() + ":不可以进行网络控制");
//
//            String myurl = "http://192.168.1.253/userRpm/AccessCtrlAccessRulesRpm.htm?enable=" + request.getSwitchCtrl() + "&enableId=" + ctrlId + "&Page=1";
//            Map<String, String> headers = new LinkedHashMap<>();
//            headers.put("Connection", "keep-alive");
//            headers.put("Upgrade-Insecure-Requests", "1");
//            headers.put("User-Agent", "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.129 Safari/537.36");
...
//
//            String ret = HttpUtil.get(myurl, headers);
//            Assert.notNull(ret, request.getAlias() + ":网络控制时出现异常");
//
//
//            String rowarr[] = null;
//
...
//
//
//            Integer switchCtrl = map.get(request.getAlias());
//            Assert.notNull(switchCtrl, request.getAlias() + ":网络控制时出现异常");
//
//            Assert.isTrue(request.getSwitchCtrl() == switchCtrl, request.getAlias() + ":网络控制时出现异常,未达到预期");
//
//
//        } catch (Exception e) {
//            log.error("获取统计数据时出现异常", e);
